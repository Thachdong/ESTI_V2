...
export const preOrderApi = {
	...
	getChartData: (id: string) => request.get<any>(`${BASE_URL}/GetHeaderPercentOrder?salesId=${id}`),

	cancel: (preOrderId: string) => request.post<any, any>(`${BASE_URL}/CancelOrder?preOrderId=${preOrderId}&status=4`, {}),

	delete: (id: string) => request.delete(`${BASE_URL}/${id}`),

	uploadFile: (file: FormData) => request.post<FormData, string>(`${BASE_URL}/upload-file`, file),
	...
	getPreOrderDetail: (id: string) => request.get<any>(`${BASE_URL}/PreOrderDetail`, { id })
}
