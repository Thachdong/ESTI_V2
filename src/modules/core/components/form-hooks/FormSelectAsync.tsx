...
export const FormSelectAsync: React.FC<TAutocompleteAsync> = (props) => {
	...
	const { isLoading, isFetching } = useQuery(
			...
			}).then((res) => res.data),
		{
			...
			onSuccess: (data) => {
				...
				const newOptions = pagination.pageIndex === 1 ? items : [...options, ...items]
	...
	const triggerLoadMoreOptions = (event: React.SyntheticEvent) => {
		...
		const isBottomReached = scrollTop + clientHeight + threshold >= scrollHeight
	...
	const onInputChange = (_: React.SyntheticEvent, value: string, reason: 'input' | 'reset' | 'clear') => {
		if (reason === 'input') {
	...
	const renderController = ({ field: { ref, ...restField }, fieldState: { error }, formState: { errors } }: TRenderControllerParams) => {
		...
		const defaultProps = {
			inputProps: {
				...inputProps,
				helperText: <ErrorMessage errors={errors} name={controlProps.name} render={({ message }) => message} />,
				error: !!error
			},
			label: updateLabel,
			labelKey,
