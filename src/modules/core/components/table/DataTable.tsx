...
export const DataTable: React.FC<TDataGrid> = ({ columns: rawCols, rows, gridProps, hideSearchbar, expandable, ...props }) => {
	// LOCAL STATES
	...
	const handleColumns = useCallback(() => {
		const cols = rawCols?.map((col) => {
			...
			if (hideSearchbar) {
				...
				column.headerClassName = '!bg-[#9FADBB]  uppercase text-white text-[12px] px-2 py-1 font-bold'
			}
	...
	return (
		<Box className={clsx('data-table-container w-full overflow-auto flex-grow h-full')}>
			<DataGrid
				headerHeight={hideSearchbar ? 40 : 64}
				getRowHeight={() => 'auto'}
				rows={rows || []}
				columns={columns}
				scrollbarSize={5}
				isRowSelectable={() => true}
				getRowClassName={({ id }) => (expandedRows.includes(id as string) ? 'expanded-row relative mb-[150px]' : '')}
