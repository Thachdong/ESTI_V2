...
export const QuoteRequestDetailTitle: React.FC = () => {
	...
	// DATA FETCHING
	const { data: requestDetail } = useQuery(['RequestDetail_', id], () => preOrderApi.getById(id as string).then((res) => res.data), {
		enabled: !!id
	})

	if (!!id) {
		const { preOrderCode, preOrderStatus, preOrderStatusName } = requestDetail?.preOrderView || {}
