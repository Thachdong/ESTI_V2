...
export const PurchasePlanProductDialog: React.FC<TDialog> = ({
  onClose,
  open,
  type,
  defaultValue,
  ...
  const getProduct = useCallback((productData: any) => {
    return {
      ...
      manufactor: productData?.manufactor,
  ...
  const handleAddProduct = useCallback((data: any) => {
    const index = products.findIndex((prod: any) => prod?.id === data?.id);
    ...
    setProductsValue("products", [...products, {...data}])
  }, [setProductsValue]);

  const handleUpdateProduct = useCallback((data: any) => {
    const updatedProducts = products.map((prod: any) => prod?.id === data?.id ? {...data} : {...prod});
    ...
    >
      <Box className="grid gap-4">
        ...
        <FormInputBase
          label="Thành tiền"
          disabled
          value={(watch("quantity") || 0) * (watch("price") || 0)}
        />
      ...
      <Box className="flex items-center justify-center mt-4">
        {type === "Add" ? (
          <BaseButton className="mr-2" onClick={handleSubmit(handleAddProduct)}>Thêm</BaseButton>
        ) : (
