	...
	return (
		<Table style={{ fontSize: '10pt' }} ref={printAreaRef} className={style.PrintContainer}>
				...
				<TableRow>
						...
						<Table sx={{ minWidth: 700 }} aria-label="customized table">
							...
							<TableBody className={style.tableBody}>
								{rows.map((row: any, idx: number) => (
									<TableRow key={`header-${row.name}-${idx}`}>
										...
										<StyledTableCell align="left" sx={{ width: '50%' }}>
											<Typography sx={{ fontWeight: 600, fontSize: '10pt' }}>{preQuoteView?.companyName}</Typography>
											<Typography sx={{ margin: 0, fontSize: '10pt' }}>
					...
					<Box className={style.InforOrder}>
						<Typography sx={{ fontSize: '10pt' }}>
							Chúng tôi xin gửi đến Quý khách hàng bảng chào giá sản phấm với nội dung chi tiết như sau:
						</Typography>
						<Table sx={{ minWidth: 700 }} aria-label="customized table">
							<TableHead>
								<TableRow>
							...
							<TableBody className={style.tableBody}>
								{preQuoteDetailView?.map((row: any, index: number) => (
									<TableRow key={`data-${row.name}-${index}`}>
										<StyledTableCell component="th" scope="row">
											{index + 1}
										</StyledTableCell>
										<StyledTableCell align="left">{row?.productCode}</StyledTableCell>
										<StyledTableCell component="th" scope="row">
											<Typography sx={{ margin: 0, fontSize: '10pt' }}>{row?.productName}</Typography>
											<Typography sx={{ margin: 0, fontSize: '10pt' }}>Nhà sản xuất: {row?.manufactor}</Typography>
										...
										<StyledTableCell align="left"> {_format.getVND(row?.totalPrice)}</StyledTableCell>
										<StyledTableCell component="th" scope="row">
											{' '}
											{row?.note}
										</StyledTableCell>
						...
						<Box className={style.paymentOrder}>
							<Box className={style.paymentItemOrder}>
								<span className={style.title}>Thành tiền chưa có thuế(VNÐ):</span>{' '}
								<span>{_format.getVND(preQuoteView?.totalPriceNotTax)}</span>
							</Box>
							<Box className={style.paymentItemOrder}>
								<span className={style.title}>Thuế GTGT:</span> <span>{_format.getVND(preQuoteView?.totalTax)}</span>
							</Box>
							<Box className={style.paymentItemOrder}>
								<span className={style.title}>Tổng cộng tiền thanh toán(VNÐ):</span>{' '}
			...
			<Box sx={{ marginBottom: 1 }}>
				<Typography sx={{ fontWeight: 600, fontSize: '10pt' }}>Điều khoản đơn hàng</Typography>
				<Typography sx={{ fontSize: '10pt', paddingLeft: 2 }}>
					- Tổng cộng tiền thanh toán đã bao gồm thuế GTGT, chi phí giao hàng và các khoản phí khác (nếu có)
				</Typography>
				<Typography sx={{ fontSize: '10pt', paddingLeft: 2 }}>- Hình thức thanh toán: {preQuoteView?.paymentType}</Typography>
				<Typography sx={{ fontSize: '10pt', paddingLeft: 2 }}>
					- Thời gian giao hàng dự kiến {_format.converseDate(preQuoteView?.deliverDate)}
				</Typography>
				<Typography sx={{ fontSize: '10pt', paddingLeft: 2 }}>
					- Hiệu lực báo giá: đến {_format.converseDate(preQuoteView?.expireDate)}
				</Typography>
				<Typography sx={{ fontSize: '10pt', paddingLeft: 2 }}>- Ðịa điểm giao hàng: {preQuoteView?.receiverAddress}</Typography>
				<Typography sx={{ fontSize: '10pt', paddingLeft: 2 }}>
					- Chứng từ thanh toán:{' '}
					{JSON.parse(preQuoteView?.paymentDocument || '[]').map((item: any) => item?.paymentDocumentName + ',')}
				</Typography>
			</Box>

			<Box sx={{ marginBottom: 1 }}>
				<Typography sx={{ fontWeight: 600, fontSize: '10pt' }}>Thông tin tài khoản ngân hàng:</Typography>
				<Typography sx={{ fontSize: '10pt', paddingLeft: 2 }}>- Ðơn vị thụ hưởng: CONG TY TNHH NAVIS VIET NAM</Typography>
				<Typography sx={{ fontSize: '10pt', paddingLeft: 2 }}>- Tài khoản : 007.1000.802.734</Typography>
				<Typography sx={{ fontSize: '10pt', paddingLeft: 2 }}>- Ngân hàng: VIETCOMBANK HCM, CN. TÂY SÀI GÒN</Typography>
			</Box>
