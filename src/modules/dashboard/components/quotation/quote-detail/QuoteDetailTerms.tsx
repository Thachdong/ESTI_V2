...
export const QuoteDetailTerms: React.FC<TProps> = ({ disabled }) => {
	...
	const { data } = useQuery(['PaymentDocument'], () => paymentDocument.getList().then((res) => res.data))

	return (
		<Box className="flex flex-col">
			<Typography className="font-bold uppercase mb-4 text-sm">ĐIỀU KHOẢN CỦA ĐƠN ĐẶT HÀNG</Typography>

			<Box className="bg-white rounded flex-grow p-3 ">
				<List className="p-0">
					<ListItem disableGutters className="pb-0 text-sm">
						- Tổng giá trị báo giá đã bao gồm thuế GTGT, chi phí giao hàng và các khoản phí, lệ phí khác (nếu có).
					</ListItem>
					...
					<ListItem disableGutters className="pb-0 text-sm">
						- Thời gian giao hàng: từ 1-3 ngày làm việc đối với hàng hoá có sẵn, chi tiết lịch giao hàng sẽ gửi qua email và cập
						nhập trên hệ thống tra cứu đơn hàng.
					</ListItem>
					...
					<ListItem disableGutters className="text-sm">
						...
						<FormSelect
							...
							controlProps={{
								...
								rules: { required: 'Chứng từ thanh toán không được để trống' }
							}}
							className="min-w-[200px] ml-2"
					...
					<ListItem disableGutters className="pb-0 text-sm">
						- Quý khách hàng xem điều khoản bảo hành tại website: www.navis.com.vn.
					</ListItem>
