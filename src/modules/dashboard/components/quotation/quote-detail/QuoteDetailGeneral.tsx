...
import { branchs, preOrderApi, staff } from 'src/api'
import { FormDatepickerBase, FormSelect, FormSelectAsync } from '~modules-core/components'

type TProps = {
	createdRequestDate?: number
}

export const QuoteDetailGeneral: React.FC<TProps> = ({ createdRequestDate }) => {
				...
				{isQuoteRequest && (
					<>
						<FormSelectAsync
							...
							fetcher={preOrderApi.getList}
							fetcherParams={{ statusPreOrder: 0 }}
							label="Mã yêu cầu báo giá"
							labelKey="preOrderCode"
						/>

						<FormDatepickerBase label="Ngày YC" value={createdRequestDate} disabled={true} />
					</>
				)}
