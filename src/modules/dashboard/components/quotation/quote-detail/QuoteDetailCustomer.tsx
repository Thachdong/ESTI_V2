...
export const QuoteDetailCustomer: React.FC<TProps> = ({ disabled }) => {
	...
	// DATA FETCHING
	const { data } = useQuery(['customerDetail_' + customerId], () => customerApi.getById(customerId).then((res) => res.data), {
		enabled: !!customerId
	})
	...
	return (
		<Box className="flex flex-col">
			...
			<Box className="grid gap-3 bg-white rounded p-3">
				...
				<FormInputBase label="Địa chỉ khách hàng" multiline minRows={2} disabled value={customer?.address} />
			</Box>
		</Box>
	)
}
