...
import { BaseButton, Dialog, FormInput, FormInputBase, FormSelect, FormSelectAsync } from '~modules-core/components'
...
export const QuoteDetailDialog: React.FC<TDialog> = ({ onClose, open, type, defaultValue }) => {
	...
	const title = type === 'Add' ? 'Thêm sản phẩm báo giá' : 'Cập nhật sản phẩm báo giá'

	// DATA FETCHING
	const { data: stock = [] } = useQuery(['GetStock', productId], () => preQuote.getProductStock([productId]).then((res) => res.data), {
		enabled: !!productId
	})
		...
		(data: any) => {
			const updatedProducts = products.map((prod: any) => {
				...
				if (!!id) {
					...
					return productId === data?.productId ? { ...product, ...data } : { ...prod }
				}
			})
	...
	return (
		<Dialog onClose={onClose} open={open} maxWidth="md" title={title} headerClassName="text-center">
			<Box className="grid grid-cols-2 gap-4 mb-4">
				<FormSelectAsync
					...
					controlProps={{
						control,
						name: 'productId',
						rules: { required: 'Phải chọn sản phẩm' }
					}}
					callback={callback}
				...
				<FormInputBase value={product?.manufactor} label="Hãng sản xuất" disabled />
				...
				<FormSelect
					controlProps={{
						control,
						name: 'vat',
						rules: { required: 'Phải chọn thuế GTGT' }
					}}
					options={VAT}
					label="Thuế GTGT"
				/>
			...
			<Box className="flex items-center justify-center mt-4">
				{type === 'Add' ? (
					<BaseButton onClick={handleSubmit(addProduct, onErrorForm)}>Thêm</BaseButton>
				) : (
					<BaseButton onClick={handleSubmit(updateProduct, onErrorForm)}>Cập nhật</BaseButton>
				)}
