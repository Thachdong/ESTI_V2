import { Box, ButtonBase, Checkbox, FormControlLabel, List, ListItem, Typography } from '@mui/material'
...
import { AddButton, ContextMenuWrapper, DataTable, DropdownButton, FormInputNumber } from '~modules-core/components'
...
export const QuoteDetailProduct: React.FC<TProps> = ({ data, disabled, refetch }) => {
	...
	const getPrice = useMemo(() => {
		if (data) {
			...
			return products.reduce((result: any, { price = 0, quantity = 0, vat = 0 }: any) => {
	...
	const mutateSelect = useMutation((payload: { id: string; selected: boolean }) => preQuote.selectProduct(payload), {
		onSuccess: (data: any) => {
	...
	if (!!id) {
		columns.splice(
			...
			},
			{
				...
				minWidth: 120,
				renderCell: ({ row }) => (
					<ButtonBase onClick={() => onOpen('askPrice')} className="bg-main text-white rounded-md p-1">
						Tạo hỏi giá
					</ButtonBase>
				)
			}
	...
	return (
		<Box className="flex flex-col col-span-2">
			...
			<Box className="bg-white">
				<ContextMenuWrapper
					...
					menuComponent={
						<Menu className="p-0" id="product_table_menu">
							...
							<Item disabled={disabled || !!id} id="delete-product" onClick={handleDelete}>
								Xóa
							</Item>
						</Menu>
					}
				...
				<List className="border-0 border-t border-solid border-grey-3 p-0">
					<ListItem className="text-sm grid grid-cols-5 items-center gap-3 py-1 border-b border-0 border-dashed border-grey-3">
						...
						<span className="text-base text-main"> {_format.getVND(getPrice.totalPrice)}</span>
					</ListItem>

					<ListItem className="text-sm grid grid-cols-5 items-center gap-3 py-1 border-b border-0 border-dashed border-grey-3">
						<span className="font-semibold col-span-4 text-right">Thuế GTGT(VNĐ):</span>{' '}
						<span className="text-base text-main"> {_format.getVND(getPrice.totalTax)}</span>
					</ListItem>

					<ListItem className="text-sm grid grid-cols-5 items-center gap-3 py-1 border-b border-0 border-dashed border-grey-3">
						<span className="font-semibold col-span-4 text-right">Tổng cộng tiền thanh toán(VNĐ):</span>
						<span className="text-base text-main"> {_format.getVND(getPrice.finalPrice)}</span>
					</ListItem>

					<ListItem className="text-sm grid grid-cols-5 items-center gap-3 py-1">
						<span className="font-semibold col-span-4 text-right">Tiền hoa hồng(VNĐ):</span>
						<FormInputNumber
							controlProps={{
								control,
								name: 'commission'
			...
			<AskPriceDialog onClose={onClose} open={!!dialog?.open && dialog?.type === 'askPrice'} defaultValue={defaultValue.current} />
		</Box>
	)
}
