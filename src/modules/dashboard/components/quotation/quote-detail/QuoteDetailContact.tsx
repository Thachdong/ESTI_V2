...
export const QuoteDetailContact: React.FC<TProps> = ({ disabled }) => {
	...
	useEffect(() => {
		if (!id && !!curator) {
			const { curatorName, curatorPhone, curatorEmail, curatorDepartment, receiverById } = curator || {}
		...
		<Box className="flex flex-col">
			...
			<Box className="bg-white rounded grid gap-3 p-3">
				<FormSelect
					...
					getOptionLabel={(opt) =>
						!!opt ? `${opt?.curatorName} - ${opt?.accountCode} - ${opt?.statusName} - ${opt?.accountTypeName}` : ''
					}
