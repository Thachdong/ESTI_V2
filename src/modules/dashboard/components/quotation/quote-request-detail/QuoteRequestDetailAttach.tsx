			...
			return (
				<List className="grid gap-2">
					{attachFile?.map((file: string) => (
						<ListItem key={file} className="flex justify-between py-0 px-3 bg-[#f4f6f8] gap-2 rounded truncate" disableGutters>
							<Typography
								component="a"
								href={file}
								target="_blank"
							...
							>
								<AttachFileIcon className="rotate-90 text-xl" /> <span className="truncate">{file}</span>
							</Typography>
