...
export const OrderDetailAttach: React.FC<TProps> = ({ disabled }) => {
	...
	const renderAttachFile = useCallback(() => {
		if (!attachFile || attachFile?.length === 0) {
			return <Typography className="text-center text-grey my-3">Không có file đính kèm</Typography>
		} else {
			return (
				<List className="grid gap-2">
					{attachFile?.map((file: string) => (
						<ListItem key={file} className="flex justify-between py-0 px-3 bg-[#f4f6f8] gap-2 rounded " disableGutters>
							<Typography
								component="a"
								href={file}
								target="_blank"
							...
							>
								...
								<span className="text-ellipsis overflow-hidden whitespace-nowrap">{file}</span>
							</Typography>
	...
	const renderTitle = (loading: boolean) => (
		...
		>
			<AttachFileIcon className="rotate-45 text-sm font-semibold" />
