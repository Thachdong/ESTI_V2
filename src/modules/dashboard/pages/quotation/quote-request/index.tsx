...
import { preOrderApi } from 'src/api/quote-request'
import {
	AddButton,
	ContextMenuWrapper,
	DataTable,
...
import { QuoteRequestHeader, ViewListProductDrawer } from '~modules-dashboard/components'
...
export const QuotationRequestsPage = () => {
	...
	// METHODS
	const muatateCancel = useMutation((id: string) => preOrderApi.cancel(id), {
		onSuccess: (data) => {
	...
	const muatateDelete = useMutation((id: string) => preOrderApi.delete(id), {
		onSuccess: (data) => {
			toast.success(data?.resultMessage)
	...
	const columns: TGridColDef[] = [
		...
		},
		{
			...
			renderCell: ({ row }) => {
				...
				const color = curatorStatus !== null ? colors[curatorStatus] : 'secondary'
				return <StatusChip status={curatorStatus} label={curatorStatusName} color={color as any} />
			}
		},
		{
			field: 'salesCode',
		...
		},
		{
			...
			renderCell: ({ row }) => (
				<DropdownButton
					...
					items={[
						{
							action: () => router.push(`quote-request-detail?id=${defaultValue?.id}`),
							label: 'Nội dung chi tiết'
						},
						{
							action: () => handleRedirect(`quote-detail?fromRequestId=${defaultValue?.id}`),
							label: 'Tạo báo giá',
	...
	const contextMenu = (
		<Menu className="p-0" id="quote-request_table_menu">
			<Item id="view" onClick={() => router.push(`quote-request-detail?id=${defaultValue?.id}`)}>
				Nội dung chi tiết
			</Item>
			<Item
				id="note"
				disabled={defaultValue?.preOrderStatus > 0}
				onClick={() => handleRedirect(`quote-detail?fromRequestId=${defaultValue?.id}`)}
			>
				Tạo báo giá
			</Item>
	...
	const handleViewProduct = async (e: React.MouseEvent<HTMLElement>) => {
		...
		const currentRow = await preOrderApi.getPreOrderDetail(id).then((res) => {
			return res.data
		})
			...
			<Paper className="bgContainer">
				<Box className="flex items-center flex-wrap gap-2 w-full justify-between mb-3">
					<Box className="flex gap-3">
						<AddButton onClick={() => router.push('quote-request-detail')} className="">
							Tạo yêu cầu
						</AddButton>
					...
					<Box className="flex gap-2">
						<StatisticButton onClick={() => setViewReport(!ViewReport)} View={ViewReport} />
